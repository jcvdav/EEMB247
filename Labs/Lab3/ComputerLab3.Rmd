...
1. **Solve for the equilibrium through simulation:**
+ Create an R script that solves these equation using `deSolve`. This requires chosing values for all of the the parameters, and initial conditions for the state variables $N4 and $P$. Let $I_N = 10$, $d_N = 0.2$, $a = 0.01$, $c = 0.5$, $d_P = 0.1$, with $N(0) = 2$ and $P(0) = 2$.

    The prey population levels off at a value of 30, and the predator population levels off at a value of 20.  If you start the model at these equilibrium values $N(0) = 30$ and $P(0) = 20$, the populations should remain at those densities.  For this combination of parameters, the model has a stable equilibrium solution at $N^* = 30$, and $P^* = 20$, at which the rates of change of both populations are equal to zero.
...
Take some pen and paper and calculate the values of the state variables, $H^*$ and $P^*$ for which:  
$H_{t+1} = H_t = H^*$  
and  
$P_{t+1} = P_t = P^*$  
...
```{r, eval = F}
setwd("/Users/Cherie/Dropbox/courses/EEMB247")
```
