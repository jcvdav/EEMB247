---
title: "EEMB247/BMSE247: Computer lab 7: Fitting models to data"
...
fixed population size. This corresponds to **frequency-dependent transmission** in which we assume that the average number of contacts per individual
is constant. 
...
where $\beta$ is the tranmission rate (in units of $\text{time}^{-1}$), $\gamma$ is the rate at which infected individuals confine themselves to bed (in units of $\text{time}^{-1}$), and $\delta$ is the rate of recovery of out of the "confined to bed class" (in units of $\text{time}^{-1}$; note, the individuals only recover after having confined themselves to bed). The question we want to ask is: **Given the data, what are reasonable estimates for $\beta$, $\gamma$ and $\delta$?**
...
> returns a list of the right hand side of the ODEs. 
> Assume that the total population size ($N$) is fixed at 763. 
>
...
$$
L(\mu = 5, \sigma = 2  \mid y_{1, \text{obs}}, y_{2, \text{obs}}, \dots, y_{100, \text{obs}}) = \prod_{t=1}^{100} \text{Normal}(y_{t, \text{obs}}  \mid \mu = 5, \sigma = 2)
$$
...
$$
-\log(L(\Theta  \mid Y)) = -\sum_{t = 1}^n \log(p(y_t  \mid \Theta))
$$
...
Consider the following measurement error model where SIBR$_{\text{B}, t \mid \beta, \gamma, \delta}$ is the SIBR model prediction for the number of boys confined to bed ($B$) at time $t$ given the parameters $\beta, \gamma, \delta$. 

$$
\begin{align}
  B_t &= \text{SIBR}_{\text{B}, t  \mid \beta, \gamma, \delta} \\
...
With this co-variance matrix, we can use brute force simulation to get the uncertainty around $R_0$.    
...
> ### HOMEWORK: Compare an SIR to the SIBR model
>
